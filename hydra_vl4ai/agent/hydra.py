...
class HydraTrainingRL(Hydra):
    ...
    def __init__(self):
        ...
        prompt_path = get_hydra_root_folder() / "agent" / "prompt" / prompt_type
        if not prompt_path.exists():
            raise NotImplementedError(f"Prompt for {prompt_type} on {self.dataset} is not implemented in {prompt_path}.")
    ...
    async def __call__(self, image: bytes, query: str, ground_true=None) -> str:
        state_memory_bank = StateMemoryBank()
        async with websockets.connect(f"ws://localhost:{Config.base_config['executor_port']}/ws/") as websocket:
            match self.task:
                case "grounding":
                    return await self._call_grounding(image, query, state_memory_bank, websocket, ground_true)

                case "vqa":
                    return await self._call_vqa(image, query, state_memory_bank, websocket, ground_true)
        ...
        with N.util.Timer(verbose=False) as timer:
            ...
            for current_step_index in range(1, self.max_iterations + 1):
                ...
                if current_step_index >1:
                    # store transition 
                    self.controller.replay_buffer.push(pre_obs_emb, selectec_idx, sub_reward, response_emb, done=False)
                ...
                if len(self.controller.replay_buffer) > self.controller.batch_size:
                    for i in range(self.controller.update_times):
                        self.controller.rl_agent_model.update(replay_buffer=self.controller.replay_buffer, batch_size=self.controller.batch_size)

                pre_obs_emb = response_emb  # reserve current emb as previous emb 
                self.controller.dqn_explore_thre_value = self.controller.dqn_explore_epsilon - self.controller.dqn_explore_epsilon_decay_rate \
            ...
            for current_step_index in range(1, self.max_iterations + 1):
                ...
                if result.type == "final":
                    if 'okvqa' in self.dataset:
                        sub_reward += GQAeval().accuracy_one_set([result.final_result], [ground_true])
                    else:
                        sub_reward += GQAeval().accuracy_one_one([result.final_result], [ground_true])
