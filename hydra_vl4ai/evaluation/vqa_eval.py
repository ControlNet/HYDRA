...
class VQAEval:
    ...
    def evaluate(self, quesIds=None):
        ...
        for quesId in quesIds:
            ...
            print(f'ori: {resAns}')
            resAns = stem_sentence(resAns)  # new
            print(f'post: {resAns}')
            gtAcc = []
            ...
            if len(set(gtAnswers)) > 1:
                for ansDic in gts[quesId]['answers']:
                    ansDic['answer'] = self.processPunctuation(
                        ansDic['answer'])
                    ansDic['answer'] = stem_sentence(ansDic['answer'])  # new
...
class GQAeval:
    ...
    def accuracy_one_one(self, prediction, ground_truth, *args):
        """
        Args:
    ...
    def accuracy_one_set(self, prediction, ground_truth, *args, print_model=False):
        """
        Args:
                prediction (list): List of predicted answers.
                ground_truth (list): List of ground truth answers.
        ...
        for p, g in zip(prediction, ground_truth):
            stem_prediction = stem_sentence(self.post_process(p))
            stem_ground_truth = [stem_sentence(
                self.post_process(sub_answer)) for sub_answer in g]
            if print_model:
